// Client-side mock data - mirrors api/mock-data.ts for static deployments

const TENANT = { id: 1, name: "Napoli Pizza", slug: "napoli-pizza" };
...
const TABLES = [
  { id: 1, tenantId: 1, number: "1", capacity: 2, section: "Patio", status: "available" as const, createdAt: new Date() },
  { id: 2, tenantId: 1, number: "2", capacity: 2, section: "Patio", status: "available" as const, createdAt: new Date() },
  { id: 3, tenantId: 1, number: "3", capacity: 4, section: "Patio", status: "occupied" as const, createdAt: new Date() },
  { id: 4, tenantId: 1, number: "4", capacity: 4, section: "Patio", status: "available" as const, createdAt: new Date() },
  { id: 5, tenantId: 1, number: "5", capacity: 6, section: "Patio", status: "reserved" as const, createdAt: new Date() },
  { id: 6, tenantId: 1, number: "6", capacity: 2, section: "Main Dining", status: "available" as const, createdAt: new Date() },
  { id: 7, tenantId: 1, number: "7", capacity: 4, section: "Main Dining", status: "occupied" as const, createdAt: new Date() },
  { id: 8, tenantId: 1, number: "8", capacity: 4, section: "Main Dining", status: "available" as const, createdAt: new Date() },
  { id: 9, tenantId: 1, number: "9", capacity: 4, section: "Main Dining", status: "reserved" as const, createdAt: new Date() },
  { id: 10, tenantId: 1, number: "10", capacity: 6, section: "Main Dining", status: "available" as const, createdAt: new Date() },
  { id: 11, tenantId: 1, number: "11", capacity: 8, section: "Main Dining", status: "available" as const, createdAt: new Date() },
  { id: 12, tenantId: 1, number: "12", capacity: 2, section: "Bar Area", status: "occupied" as const, createdAt: new Date() },
  { id: 13, tenantId: 1, number: "13", capacity: 2, section: "Bar Area", status: "available" as const, createdAt: new Date() },
  { id: 14, tenantId: 1, number: "14", capacity: 4, section: "Bar Area", status: "available" as const, createdAt: new Date() },
  { id: 15, tenantId: 1, number: "15", capacity: 4, section: "Private Room", status: "available" as const, createdAt: new Date() },
  { id: 16, tenantId: 1, number: "16", capacity: 8, section: "Private Room", status: "reserved" as const, createdAt: new Date() },
  { id: 17, tenantId: 1, number: "17", capacity: 12, section: "Private Room", status: "available" as const, createdAt: new Date() },
  { id: 18, tenantId: 1, number: "18", capacity: 2, section: "Counter", status: "available" as const, createdAt: new Date() },
];
...
const mockHandlers: Record<string, (input: any) => any> = {
  "menu.categories": () => CATEGORIES.filter(c => c.isActive).sort((a, b) => a.sortOrder - b.sortOrder),
  "menu.items": ({ categoryId }: { categoryId?: number }) => {
    let items = MENU_ITEMS.filter(i => i.isAvailable);
    if (categoryId) items = items.filter(i => i.categoryId === categoryId);
    return items.sort((a, b) => a.name.localeCompare(b.name));
  },
  "menu.itemById": ({ id }: { id: number }) => MENU_ITEMS.find(i => i.id === id) ?? null,
  "menu.popular": () => MENU_ITEMS.filter(i => i.isAvailable && i.isPopular),
  "order.list": ({ status }: { status?: string }) => {
    let orders = [...ORDERS];
    if (status) orders = orders.filter(o => o.status === status);
    return orders.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  },
  "order.byId": ({ id }: { id: number }) => {
    const order = ORDERS.find(o => o.id === id);
    if (!order) return null;
    const items = ORDER_ITEMS.filter(i => i.orderId === id);
    return { ...order, items };
  },
  "order.today": () => ORDERS.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()),
  "kitchen.tickets": () => {
  ...
  "inventory.list": ({ search, lowStock }: { search?: string; lowStock?: boolean }) => {
  ...
  "inventory.categories": () => [...new Set(INVENTORY.map(i => i.category))].filter(Boolean),
  "staff.list": ({ role }: { role?: string }) => {
    let items = [...STAFF];
  ...
  "analytics.popularItems": ({ limit = 5 }: { limit?: number }) => {
  ...
  "rbac.myPermissions": () => PERMISSIONS.map(p => ({ ...p, granted: true })),
  "rbac.allPermissions": () => [...PERMISSIONS],
};
...
export { CATEGORIES, MENU_ITEMS, ORDERS, ORDER_ITEMS, STAFF, TABLES, INVENTORY, PERMISSIONS };
