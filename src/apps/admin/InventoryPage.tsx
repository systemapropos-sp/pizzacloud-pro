...
export default function InventoryPage() {
  ...
  const { data: items, isLoading } = useInventory(search || undefined, showLow || undefined);

  return (
    <div className="p-6">
      <div className="mb-6"><h1 className="text-2xl font-bold font-serif text-gray-900">Inventory</h1><p className="text-gray-500">{items?.length ?? 0} items in stock</p></div>
      <div className="flex gap-4 mb-6">
        <div className="relative flex-1 max-w-sm"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><input type="text" placeholder="Search items..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-9 pr-3 py-2.5 border rounded-lg text-sm" /></div>
        ...
        <div className="bg-white rounded-xl border overflow-hidden">
          <div className="overflow-x-auto"><table className="w-full text-sm">
            <thead className="bg-gray-50 border-b"><tr>
              ...
              <th className="text-left px-4 py-3 font-medium text-gray-500">Unit Cost</th>
            ...
            <tbody>{items?.map(item => {
              ...
              return (
                <tr key={item.id} className="border-b hover:bg-gray-50">
                  <td className="px-4 py-3 font-medium flex items-center gap-2"><Package className="w-4 h-4 text-gray-400" />{item.name}</td>
                  ...
                  <td className="px-4 py-3 text-gray-500">${item.unitCost}</td>
