...
import { usePosMenuItems } from "@/hooks/useStaticQueries";
...
export default function PosTerminal() {
  ...
  const processPay = () => {
    ...
    const orderNumber = `PZ-${Date.now().toString(36).toUpperCase().slice(-6)}`;
    ...
    <div className="flex h-full">
      <div className="flex-1 p-4 overflow-auto">
        ...
        <div className="grid grid-cols-3 lg:grid-cols-4 gap-3">
          {filtered?.map(item => (
            <button key={item.id} onClick={() => quickAdd(item)} className="bg-white rounded-xl border p-3 text-left hover:shadow-md hover:border-red-300 transition-all active:scale-95">
              ...
              <p className="text-[10px] text-gray-400">{item.prepTimeMinutes}min</p>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="w-96 bg-white border-l flex flex-col">
        <div className="p-4 border-b"><h2 className="font-bold text-lg">Current Order</h2><p className="text-xs text-gray-400">{items.length} items</p></div>
